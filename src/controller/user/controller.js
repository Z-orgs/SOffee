...
function getMember(req, res) {
	return __awaiter(this, void 0, void 0, function* () {
		try {
			...
			const member = Object.assign(Object.assign({}, result), {
				dob: (0, function_1.formatDate)(
					result === null || result === void 0 ? void 0 : result.dob,
				),
			});
	...
	return __awaiter(this, void 0, void 0, function* () {
		try {
			if (req.files) {
				...
				const uploadPath =
					app_root_path_1.default + '/src/public/files/' + image.name;
	...
	return __awaiter(this, void 0, void 0, function* () {
		try {
			const [member, guest] = yield Promise.all([
				SOffee_1.default.Member.findOne({
					username: req.signedCookies.username,
				}),
				SOffee_1.default.Guest.findOne({
					username: req.signedCookies.username,
				}),
			]);
			let cart = member
				? [...member.cart]
				: [
						...((guest === null || guest === void 0
							? void 0
							: guest.cart) !== undefined
							? guest === null || guest === void 0
								? void 0
								: guest.cart
							: []),
				  ];
			cart = yield Promise.all(
				cart.map((item) =>
					__awaiter(this, void 0, void 0, function* () {
						try {
							const data =
								yield SOffee_1.default.Product.findById(
									item.productId,
								);
							item.product = Object.assign({}, data);
						} catch (err) {
							console.log(err);
						} finally {
							return item;
...
function addToCart(req, res) {
	try {
		SOffee_1.default.Product.findById(req.body.productId)
			.then((data) => {
				if (
					!(data === null || data === void 0 ? void 0 : data.quantity)
				) {
				...
				if (data.quantity < req.body.quantity) {
					...
					SOffee_1.default.Guest.updateOne(
						{ username: req.signedCookies.username },
						{
							cart: [
								{
...
function submitCart(req, res) {
	return __awaiter(this, void 0, void 0, function* () {
		...
		for (const product of req.body.submitProduct) {
			let result = yield SOffee_1.default.Product.findById(
				product.productId,
			);
...
function getBill(req, res) {
	SOffee_1.default.Bill.find({ username: req.signedCookies.username }).then(
		(data) => {
			let result = [...data];
...
function submitBill(req, res) {
	console.log('a');
	SOffee_1.default.Bill.updateOne(
		{ _id: req.body.billId },
		{
			$set: {
				isSend: true,
		...
		.then((data) =>
			__awaiter(this, void 0, void 0, function* () {
				...
				yield SOffee_1.default.Guest.updateOne(
					{ username: req.signedCookies.username },
					{
						$set: {
							cart: [],
